...
public:
    ...
    double run() {
        ...
        vkCmdResetQueryPool(commandBuffer_, queryPool_, 0, 7);
        ...
        if (mode_ == 1u) {
            ...
            hostToShaderBarrier(yuvDownsampled_.buffer, yuvDownsampled_.size);
        } else {
            recordDownsample();
            vkCmdWriteTimestamp(commandBuffer_, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, queryPool_, 1);
        ...
        barrier(yuvFiltered_.buffer, yuvFiltered_.size,
                VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT);
        vkCmdWriteTimestamp(commandBuffer_, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, queryPool_, 6);
        ...
        commandMs_ = std::chrono::duration<double, std::milli>(end - start).count();

        std::array<uint64_t, 7> timestamps{};
        check(vkGetQueryPoolResults(device_, queryPool_, 0, 7,
                                    sizeof(timestamps), timestamps.data(),
                                    sizeof(uint64_t),
                                    VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT),
              "vkGetQueryPoolResults");
        const double periodMs = properties_.limits.timestampPeriod / 1'000'000.0;
        downsampleMs_ = static_cast<double>(timestamps[1] - timestamps[0]) * periodMs;
        downsampleToSobelBarrierMs_ = static_cast<double>(timestamps[2] - timestamps[1]) * periodMs;
        sobelMs_ = static_cast<double>(timestamps[3] - timestamps[2]) * periodMs;
        sobelToMedianBarrierMs_ = static_cast<double>(timestamps[4] - timestamps[3]) * periodMs;
        medianMs_ = static_cast<double>(timestamps[5] - timestamps[4]) * periodMs;
        finalGpuToCpuBarrierMs_ = static_cast<double>(timestamps[6] - timestamps[5]) * periodMs;
        return commandMs_;
    }
    ...
    void copyOutputTo(JNIEnv* env, jbyteArray outputArray) {
        ...
        void* mapped = nullptr;
        check(vkMapMemory(device_, yuvFiltered_.memory, 0, yuvFiltered_.size, 0, &mapped),
              "vkMapMemory(final)");
        env->SetByteArrayRegion(outputArray, 0, static_cast<jsize>(bytes),
                                static_cast<const jbyte*>(mapped));
        vkUnmapMemory(device_, yuvFiltered_.memory);
    }
    ...
    std::string report(double gpuMs) const {
        ...
        out << properties_.deviceName << '\n'
            ...
            << "Intermediate buffers stay native/GPU-side:\n"
            ...
            << (mode_ == 1u ? " (host write -> shader read)" : " (shader write -> shader read)") << '\n'
            << "  Sobel mask: " << sobelMs_ << " ms\n"
            << "  Barrier sobel -> median_v2: " << sobelToMedianBarrierMs_ << " ms\n"
            << "  Median_v2: " << medianMs_ << " ms\n"
            << "  Barrier median_v2 -> CPU read: " << finalGpuToCpuBarrierMs_ << " ms\n"
...
private:
    ...
    uint32_t findMemoryType(uint32_t bits, VkMemoryPropertyFlags preferred) const {
        ...
        for (uint32_t i = 0; i < memoryProperties_.memoryTypeCount; ++i) {
            const auto flags = memoryProperties_.memoryTypes[i].propertyFlags;
            if ((bits & (1u << i)) && (flags & fallback) == fallback) {
                return i;
            }
        }
    ...
    Buffer makeBuffer(VkDeviceSize size, VkBufferUsageFlags usage) {
        ...
        const VkMemoryAllocateInfo alloc{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO, nullptr,
                                         req.size, findMemoryType(req.memoryTypeBits, preferred)};
    ...
    void createBuffers() {
        ...
        for (Buffer& source : sourcePlanes_) {
            source = makeBuffer(static_cast<VkDeviceSize>(srcPlaneWords_) * sizeof(uint32_t), storage);
        }
        const VkDeviceSize compactBytes = static_cast<VkDeviceSize>(dstPlaneWords_) * 3u * sizeof(uint32_t);
        yuvDownsampled_ = makeBuffer(compactBytes, storage);
        sobelMask_ = makeBuffer(compactBytes, storage);
        yuvFiltered_ = makeBuffer(compactBytes, storage);
    }
    ...
    void uploadRgbAsYuv444() {
        if (mode_ == 1u) {
            void* mapped = nullptr;
            check(vkMapMemory(device_, yuvDownsampled_.memory, 0, yuvDownsampled_.size, 0, &mapped),
                  "vkMapMemory(yuvDownsampled)");
            ...
            vkUnmapMemory(device_, yuvDownsampled_.memory);
            return;
        }
        ...
        for (uint32_t c = 0; c < 3u; ++c) {
            void* mapped = nullptr;
            check(vkMapMemory(device_, sourcePlanes_[c].memory, 0, sourcePlanes_[c].size, 0, &mapped),
                  "vkMapMemory(source)");
        ...
        for (uint32_t c = 0; c < 3u; ++c) {
            vkUnmapMemory(device_, sourcePlanes_[c].memory);
        }
    }
    ...
    void barrier(VkBuffer buffer, VkDeviceSize size, VkAccessFlags src, VkAccessFlags dst) {
        ...
        if (dst == VK_ACCESS_HOST_READ_BIT) dstStage = VK_PIPELINE_STAGE_HOST_BIT;
    ...
    void hostToShaderBarrier(VkBuffer buffer, VkDeviceSize size) {
        const VkBufferMemoryBarrier memoryBarrier{VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER, nullptr,
...
Java_app_builderx_ogfa_camerapipelinetest_OptimizedPreprocessingTest_nativeRunFullPipeline(
    JNIEnv* env,
    jclass,
    jobject bitmap,
